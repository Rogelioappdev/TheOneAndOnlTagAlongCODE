...
export const signInWithApple = async (): Promise<{ success: boolean; error?: string }> => {
  try {
    ...
    if (isNativeAvailable) {
      // Use native Apple Sign In on iOS with proper entitlements
      try {
        ...
        // identityToken can be null — guard before passing to Supabase
        const identityToken = credential.identityToken;
        if (!identityToken) {
          console.error("Apple Sign In: identityToken is null or missing from credential");
          return { success: false, error: "Apple did not return an identity token. Please try again." };
        }

        // Ensure the token is a clean UTF-8 string (trim any whitespace)
        const tokenString = String(identityToken).trim();
        ...
        // Re-throw other native errors rather than silently falling back to OAuth,
        // since the OAuth flow does not work in TestFlight / production builds.
        console.error("Native Apple Auth error:", nativeError);
        return { success: false, error: nativeError.message || "Apple Sign In failed. Please try again." };
      }
    }

    // Fallback: Use web-based OAuth flow (works in Expo Go and development)
    WebBrowser.maybeCompleteAuthSession();
