<stack>
  ...
  Supabase for authentication (Google OAuth, Apple Sign In), database, real-time, and storage.
...
<authentication>
  Google OAuth and Apple Sign In are integrated in onboarding flow (Slide 3 - after "Active Travelers").

  Google OAuth:
  - Authentication uses Supabase Auth with signInWithOAuth.
  - User profiles are automatically created in public.users table with Google account details.
  - Returning authenticated users skip onboarding and go directly to home.
  - Authentication state is persisted via Supabase Auth storage (AsyncStorage).

  Apple Sign In (iOS only):
  - Native iOS Apple Sign In with FaceID support via expo-apple-authentication.
  - Uses signInWithIdToken for Supabase integration.
  - Automatically handles iOS keychain and biometric authentication.
  - Apple Auth is enabled in Supabase Dashboard (Authentication > Providers > Apple).

  Deep Link Configuration:
  - Redirect URL: https://tnstvbxngubfuxatggem.supabase.co/auth/v1/callback
  - Configured in app.json with scheme: "tagalong"
  - After login, continues to next onboarding slide (does NOT go to home yet)

  IMPORTANT: For Google OAuth to work, configure in Supabase Dashboard:
  1. Go to Authentication > Providers > Google
  2. Enable Google provider
  3. Add your Google OAuth client ID and secret from Google Cloud Console
  4. Redirect URL is automatically configured in code
  5. For testing, Google OAuth works in development via expo-web-browser

  IMPORTANT: For Apple Sign In to work properly in production:
  1. Apple Auth must be enabled in Supabase Dashboard (Authentication > Providers > Apple)
  2. app.json must include Apple Sign In entitlements (managed by Vibecode during build)
  3. Bundle ID must be properly configured in Apple Developer account
  4. Works natively on iOS with biometric authentication (FaceID/TouchID)
</authentication>
